...
namespace Scadenzetti
{
    class DataAccessGateway
    {
        ...
        public void salvaRateazioneMovimento(string tipo, int idUtente, int idDest, string causale, string note, 
            List<int> categorie, List<RataMovimento> rate)
        {
            OleDbConnection conn = new OleDbConnection(Properties.Settings.Default.scadenzettiDbConnectionString);

            string queryStringMov = "INSERT INTO Movimento (Tipo, DataScadenza, ImportoIvato, Ultimato, Causale, [Note], Utente, DestinatarioMov) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            conn.Open();
            OleDbCommand cmd = new OleDbCommand(queryStringMov, conn);
            foreach (RataMovimento rt in rate)
            {
                salvaRata(rt, tipo, idUtente, idDest, "Rata "+rt.Progr+" di "+rate.Count+": "+causale, note, categorie, conn, ref cmd);
        ...
        private static void salvaRata(RataMovimento r, string tipo, int idUtente, int idDest, string causale, string note, List<int> categorie, OleDbConnection conn, ref OleDbCommand cmd)
        {
            cmd.Parameters.Add("Tipo", OleDbType.Char, 255).Value = tipo;
            cmd.Parameters.Add("DataScadenza", OleDbType.Date).Value = r.Scadenza;
            cmd.Parameters.Add("ImportoIvato", OleDbType.Double).Value = r.Importo;
            cmd.Parameters.Add("Ultimato", OleDbType.Boolean).Value = false;
            cmd.Parameters.Add("Causale", OleDbType.Char, 255).Value = causale;
            cmd.Parameters.Add("Note", OleDbType.Char, 255).Value = note;
            cmd.Parameters.Add("Utente", OleDbType.Integer).Value = idUtente;
            ...
            OleDbCommand cmdId = new OleDbCommand("SELECT @@IDENTITY", conn);
            int newIdMov = (int)cmdId.ExecuteScalar();
